    ...
    if (pv := event.is_private) or event.is_group :
        if pv:
            try:
                ...
                await event.reply(f"First join Our Channel to access the bot\n\nJoin @{Config.CHANNEL_USERNAME}\n\nAfter that /start the bot aging.")
                return
        ...
        if event.file :
            ...
            await event.reply(f"Download Link: \n\n📎 : {Config.DOMAIN}/{id}\n\n Telegram Stored Link: {bot_url}")
            return
            ...
            if id_hex := id_msg.group(1) :
                ...
                if regex := re.search(r"(\d*)/(\d*)",msg.message):
                    if user_id := int(regex.group(1)) :
                        ...
                        bot_url = f"t.me/{username_bot}?start={id_hex}"
                        forward_reply = await forward.reply(f"Download Link: {Config.DOMAIN}/{id_name}\n\nTelegram Stored Link: {bot_url}",link_preview=False)
                        await asyncio.sleep(12)
                        await forward_reply.edit(f"Download Link: {Config.DOMAIN}/{id_name}\n\nTelegram Stored Link: {bot_url}")
        ...
        if pv:
            await event.reply("Send Any File To Get a Direct Download Link And Telegram Stored Link")
